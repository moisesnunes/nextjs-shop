            ...
            <main className="md:mx-auto mx-5 max-w-5xl pb-24">
                <div className="py-5 border-b border-gray-400">
                    <h1 className="my-4 text-3xl font-semibold lg:text-4xl">
                        {items.length > 0 ? "Veja o que você tem no seu carrinho! :)" : "Seu carrinho está vazio"}
                    </h1>
                    <p className="my-4">Fretes grátis para todo brasil.</p>
                    {items.length === 0 && (
                        <BuyBtn onClick={() => router.push("/")} title="Continue comprando" />
                    )}
                </div>
                {items.length > 0 && (
                    <div className="mx-5 md:mx-8">
                        ...
                        <div className="my-12 mt-6 ml-auto max-w-3xl">
                            <div className="divid-y divid-gray-300">
                                <div className="pb-4">
                                    <div className="flex justify-between">
                                        <p>Subtotal</p>
                                        <p>R${(basketTotal).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}</p>
                                    </div>
                                </div>
                                <div className="flex justify-between">
                                    <p>Fretes Grátis</p>
                            ...
                            <div className="flex justify-between pt-4 text-xl font-semibold">
                                <h4>Total</h4>
                                <h4 className="text-2xl">R${(basketTotal).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')} </h4>
                            </div>
                            {/* Checkout */}
